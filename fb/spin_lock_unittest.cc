...
TEST(SpinLock, MslCorrectness) {
  correctnessTest<fb::SpinLockMslImpl>();
}
TEST(SpinLock, MslTryLock) {
  trylockTest<fb::SpinLockMslImpl>();
}
//#endif
//
//#if FOLLY_HAVE_PTHREAD_SPINLOCK_T
TEST(SpinLock, PthreadCorrectness) {
  correctnessTest<fb::SpinLockPthreadImpl>();
}
TEST(SpinLock, PthreadTryLock) {
  trylockTest<fb::SpinLockPthreadImpl>();
}
//#endif

TEST(SpinLock, MutexCorrectness) {
  correctnessTest<fb::SpinLockPthreadMutexImpl>();
}
TEST(SpinLock, MutexTryLock) {
  trylockTest<fb::SpinLockPthreadMutexImpl>();
}
