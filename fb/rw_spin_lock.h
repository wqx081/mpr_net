...
class RWSpinLock {
 ...
 public:
  ...
  void lock_shared() {
    int count = 0;
    while (!MPR_LIKELY(try_lock_shared())) {
      if (++count > 1000) sched_yield();
    }
  }
  ...
  bool try_lock_shared() {
    ...
    if (MPR_UNLIKELY(value & (WRITER|UPGRADED))) {
      bits_.fetch_add(-READER, std::memory_order_release);
      return false;
    }
    return true;
...
class RWTicketSpinLockT {
 ...
 public:
  ...
  void writeLockAggressive() {
    ...
    while (val != load_acquire(&ticket.write)) {
      asm volatile("pause");
      if (MPR_UNLIKELY(++count > 1000)) sched_yield();
    }
  }
  ...
  void lock_shared() {
    ...
    while (!MPR_LIKELY(try_lock_shared())) {
      asm volatile("pause");
      if (MPR_UNLIKELY((++count & 1023) == 0)) sched_yield();
    }
  }
  ...
  class ReadHolder { //: boost::noncopyable {
   ...
   private:
    ...
    DISALLOW_COPY_AND_ASSIGN(ReadHolder);
  };
  ...
  class WriteHolder { // : boost::noncopyable {
   ...
   private:
    ...
    DISALLOW_COPY_AND_ASSIGN(WriteHolder);
  };
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(RWTicketSpinLockT);
};
