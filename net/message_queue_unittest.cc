 ...
 public:
  ...
  bool IsLocked() {
    ...
    return worker.Invoke<bool>(
        FROM_HERE, Bind(&MessageQueueTest::IsLocked_Worker, this));
  }
};
...
static void DelayedPostsWithIdenticalTimesAreProcessedInFifoOrder(
  ...
  q->PostAt(FROM_HERE, now, NULL, 3);
  q->PostAt(FROM_HERE, now - 2, NULL, 0);
  q->PostAt(FROM_HERE, now - 1, NULL, 1);
  q->PostAt(FROM_HERE, now, NULL, 4);
  q->PostAt(FROM_HERE, now - 1, NULL, 2);
...
class DeletedMessageHandler : public MessageHandler {
 public:
  ...
  void OnMessage(Message* msg) { (void)msg; }
 private:
  bool* deleted_;
};
...
TEST(MessageQueueManager, Clear) {
  ...
  if (MessageQueueManager::IsInitialized()) {
    LOG(INFO) << "Unable to run MessageQueueManager::Clear test, since the "
