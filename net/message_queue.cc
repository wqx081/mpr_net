...
void MessageQueueManager::ProcessAllMessageQueuesInternal() {
  ...
  auto functor = [&queues_not_done] { base::subtle::Barrier_AtomicIncrement(&queues_not_done, -1); };
  FunctorMessageHandler<void, decltype(functor)> handler(functor);
  {
base::CritScope cs(&crit_);
    queues_not_done = static_cast<int>(message_queues_.size());
    for (MessageQueue* queue : message_queues_) {
      queue->PostDelayed(FROM_HERE, 0, &handler);
    }
  }
  ...
  while (base::subtle::Acquire_Load(&queues_not_done) > 0) {
...
void MessageQueue::PostDelayed(const base::Location& posted_from,
                               int cmsDelay,
                               MessageHandler* phandler,
                               uint32_t id,
                               MessageData* pdata) {
...
pmsg->posted_from.function_name());
  ...
  if (diff >= kSlowDispatchLoggingThreshold) {
    LOG(INFO) << "Message took " << diff << "ms to dispatch. Posted from: "
