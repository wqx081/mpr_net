...
bool IPAddress::operator <(const IPAddress &other) const {
  ...
  switch (family_) {
    case AF_INET: {
      return base::NetworkToHost32(u_.ip4.s_addr) <
          base::NetworkToHost32(other.u_.ip4.s_addr);
    }
    case AF_INET6: {
...
IPAddress TruncateIP(const IPAddress& ip, int length) {
  ...
  if (ip.family() == AF_INET) {
    ...
    uint32_t host_order_ip = base::NetworkToHost32(ip.ipv4_address().s_addr);
    in_addr masked;
...
int CountIPMaskBits(IPAddress mask) {
  ...
  switch (mask.family()) {
    case AF_INET: {
      word_to_count = base::NetworkToHost32(mask.ipv4_address().s_addr);
      break;
    }
    case AF_INET6: {
