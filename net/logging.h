...
#include "net/thread_annotations.h"
...
#define LOG_SEVERITY_PRECONDITION(sev) \
  !(net::LogMessage::Loggable(sev)) \
    ? (void) 0 \
    : net::LogMessageVoidify() &

#define LOG(sev) \
  LOG_SEVERITY_PRECONDITION(net::sev) \
    net::LogMessage(__FILE__, __LINE__, net::sev).stream()

// The _V version is for when a variable is passed in.  It doesn't do the
// namespace concatination.
#define LOG_V(sev) \
  LOG_SEVERITY_PRECONDITION(sev) \
    net::LogMessage(__FILE__, __LINE__, sev).stream()
...
#define LOG_CHECK_LEVEL(sev) \
  net::LogCheckLevel(net::sev)
#define LOG_CHECK_LEVEL_V(sev) \
  net::LogCheckLevel(sev)
...
#define LOG_E(sev, ctx, err, ...) \
  LOG_SEVERITY_PRECONDITION(net::sev) \
    net::LogMessage(__FILE__, __LINE__, net::sev, \
                          net::ERRCTX_ ## ctx, err , ##__VA_ARGS__) \
        .stream()
...
#define LOG_TAG(sev, tag) \
  LOG_SEVERITY_PRECONDITION(sev) \
    net::LogMessage(NULL, 0, sev, tag).stream()
