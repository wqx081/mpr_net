...
class SCOPED_LOCKABLE AnnounceOnThread {
 ...
 private:
  AnnounceOnThread() = delete;
  DISALLOW_COPY_AND_ASSIGN(AnnounceOnThread);
};
...
#define DCHECK_RUN_ON(thread_like_object) \
  net::internal::AnnounceOnThread thread_announcer(thread_like_object); \
  DCHECK(net::internal::AnnounceOnThread::IsCurrent(thread_like_object))
